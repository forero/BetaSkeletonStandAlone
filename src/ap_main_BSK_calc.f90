...
!	call mpi_comm_rank(mpi_comm_world,myid,ierr)
	...
	if(numarg .le. 0) then
		...
		write(*,'(A)'), ' Usage: ./BSK_calc -om omegam -w w -input intpufilename -output & 
                                 &outputfilname -beta beta -printinfo printinfo -numNNB numNNB -minimalrcut minimalr_cut &
                                 &-maximalrcut maximalr_cut -nglcrosscheck do_nglcrosscheck -ngldir ngldir &
                                 &-numdrop numdrop -dropstep dropstep'
		stop
	endif
	...
	do i = 1, numarg
		...
		elseif(trim(adjustl(tmpstr1)).eq.'-input') then
			read(tmpstr2,*) inputfilename
		elseif(trim(adjustl(tmpstr1)).eq.'-output') then
			read(tmpstr2,*) outputfilename
		...
		elseif(trim(adjustl(tmpstr1)).eq.'-dropstep') then
			...
			write(*,'(A)'), ' Usage: ./BSK_calc -om omegam -w w -input intpufilename -output outputfilname & 
                                        &-beta beta -printinfo printinfo -numNNB numNNB -minimalrcut minimalr_cut & 
                                        &-maximalrcut maximalr_cut -nglcrosscheck do_nglcrosscheck -ngldir ngldir & 
                                        &-numdrop numdrop -dropstep dropstep'
			stop
		endif
	...
	print *, '  omegam, w = ', omegam, w
	print *, '  inputfilename: ', trim(adjustl(inputfilename))
