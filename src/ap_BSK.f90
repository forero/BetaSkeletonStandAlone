...
!!! Corss-Check using ngl
     ...
     if(do_nglcrosscheck) then
        ...
        cmdstr = trim(adjustl(ngldir))//' -i '//trim(adjustl(file_xyz))//&
             &' -d 3 -b '//trim(adjustl(tmpstr1))//' >> '//trim(adjustl(file_ngl))
   ...
   subroutine BSK_stat(omegam, w, beta, outputfilename, numdrop, dropstep, printinfo)
     ...
     write(my_fmt, '(a,i0,a)') '(36x,',cs%numdrop,'(f8.3,1x))'
     write(*,my_fmt)	chisqlist(1:cs%numdrop)
     
     write(my_fmt, '(a,i0,a)') '(A,12x,',cs%numdrop,'(f8.3,1x))'
     write(*,my_fmt)  dfchisqlist(1:cs%numdrop)
     do j = 1, nbinchisq	
        write(my_fmt, '(a,i0,a)') '(4x,i4,A,24x,',cs%numdrop,'(f8.3,1x))'
        write(*,my_fmt) nbinchisqlist(j),' bins:', multdfchisqlist(j,1:cs%numdrop)
     enddo
