...
contains	
  ...
  !------------------------------------------
	logical function has_boundary_effect(x,y,z,max_dist)
		! Dummy
		real(dl) :: x,y,z,ra,dec,max_dist !max_dist is the radius of smoothing sphere
		!local 
		real(dl) :: r,dr, rdr ! An extra distance away from the boundary
...
861		continue		
		...
		do i_check = 1, 2
			...
			if(.not.(ddl>minimal_dradec .and. ddr>minimal_dradec .and. ddu>minimal_dradec .and. ddd>minimal_dradec)) then
				has_boundary_effect = .true.; return
			endif
		enddo
  ...
  !------------------------------------------
	subroutine grid_rho_drho_list(smnum, printinfo, pos_list, rho_list, drho_list)
		...
		integer :: i,j, ix, iy, iz, n, iscan, n0, n1, n2, fixmdsmnum, erflagint, insphere_numran, percent, &
			insphere_numran_min, insphere_numran_max, numransmnum(n_ransmnum), num_scan
		logical ::  erflaglog
...
!				stop
		...
		if(gb_outputinfo_gfcell) then
			...
			write(5,'(A)') '#fmt: 1 x,2 y,3 z,4 ra,5 dec,6 r,7 gb_cell_mat(ix,iy,iz)%maxdist, & 
                                                    &8 deltaradec, 9 gb_cell_mat(ix,iy,iz)%insphere_numran, & 
                                                    &10 gb_cell_mat(ix,iy,iz)%weirat, 11 gb_cell_mat(ix,iy,iz)%sumwei_acpt, &
                                                    &12 gb_cell_mat(ix,iy,iz)%sumwei_all, 13 rho, 14 drhodx, 15 drhody, 16 drhodz, &
