...
!			endif
		...
		enddo
		if(printinfo) then
                        write(my_fmt, '(a,i0,a)') '(x,A,',nbin,'(e14.7,1x))'
			write(*,my_fmt) '(muaver_jackknife) muavs = ', muavs(1:nbin)
                        write(my_fmt, '(a,i0,a)') '(19x,A,',nbin,'(e14.7,1x))'
			write(*,my_fmt) 'muers = ', muers(1:nbin)
			write(*,my_fmt) 'ravs  = ', ravs(1:nbin)
			write(*,my_fmt) 'rers  = ', rers(1:nbin)
		endif
			...
			if(gb_calc_absdrho_chisq) then
				...
				if(gb_outputinfo_absdrho) then ! only output absdrho for idrop 1
	                          write(str,*) idrop
                                  ...
                                  absdrhofile1 = trim(adjustl(gb_suboutput_name)) // &
                                       &"_absdrhoinfo_idrop" // trim(adjustl(str)) // ".txt"
                                  absdrhofile2 = trim(adjustl(gb_suboutput_name))//&
                                       &"_absrhoinfo_idrop"//trim(adjustl(str))//".txt"
                                  absdrhofile3 = trim(adjustl(gb_suboutput_name))//&
                                       &"_absdrhotorhoinfo_idrop"//trim(adjustl(str))//".txt"
