! To be check:
	! read in data/random: whether the x/y/zs are correct? whether they are correct when we change cosmologies? ! Done: Correct
	! properties of smoothing distance

! Next Step: Make the code clean and neat; 

! Next step: Read-in subroutine; Rho/Drho calculation subroutin (test particles for boundary-correction); radial smooth subroutine (normn_list)...
...
!!! Fixed Settings
	...
	integer, parameter :: gb_numcatalogue = 4
	...
	real(dl), parameter :: gb_redcut_mins(gb_numcatalogue) = (/  0.43_dl, 0.00_dl, 0.43_dl, 0.00_dl  /)
	real(dl), parameter :: gb_redcut_maxs(gb_numcatalogue) = (/  0.70_dl, 0.43_dl, 0.70_dl, 0.43_dl  /)
	real(dl), parameter :: gb_minimalr_cuts(gb_numcatalogue) = (/ 1172.7394_dl,  100.0_dl, 1172.7394_dl,  100.0_dl /)
	real(dl), parameter :: gb_maximalr_cuts(gb_numcatalogue) = (/ 1787.3546_dl, 1172.8_dl, 1787.3546_dl, 1172.8_dl /)
	real(dl), parameter :: gb_suggested_numinx(gb_numcatalogue) = (/ 120.0_dl, 83.0_dl, 68.00_dl, 78.0_dl/)
				! these numinx correspond to the following cell widths and grid sizes: 
	...
	! for data type -1: whether there is the fourth column as weight;	
	logical, parameter :: gb_hasweight = .false.
...
! All variables in the "Adjustable & To be fixed Settings" section must be assigned a value in the main**.f90!!!
	...
	! Type of data; 0: constant number density; 1: varying radial density, no RSD; 2: varying nbar, with RSD; 3: real data
	...
	real(dl) :: gb_ranwei_tol = 100.0 ! If (weight-of-acpt)/(weight-of-all) < gb_ranweitol, then reject this position.
	...
	real(dl) :: gb_Seff, gb_redcut_min, gb_redcut_max, gb_minimalr_cut, gb_maximalr_cut
...
contains
  ...
  !------------------------------------------
	logical function check_reject_radecran(ra,dec)
		...
		if(ra>228.and.ra<232.and.dec>38.and.dec<42) then
  ...
  !------------------------------------------
	subroutine readin_mockdata(printinfo)
		...
		! Local
		integer :: i, tot_numdata, numdata, ID, Andersonwei,vetowei,wei,wcp
		real(dl) :: tmp(10),wfkp,mass
  ...
  !------------------------------------------
	subroutine readin_BOSSdata(printinfo)
		...
		do while(.true.)		
			...
			if(redshift>gb_redcut_min .and. redshift<gb_redcut_max .and. &
...
100				exit
		...
		do i_ran = 1, gb_numranfile
			...
			do while(.true.)
				...
				if(iRedshiftrange.le.0.5 .or. iRadecrange.le.0.5 & !drop when redshift/radec out of range
	...
	subroutine readin_HR3radecran(printinfo)
		...
		do i_ran = 1, gb_numradecranfile
			...
			do while(.true.)
				...
				if(iRadecrange.le.0.5 .or. check_reject_radecran(ra,dec) & !drop when radec out of range or rejected
  ...
  !------------------------------------------
	subroutine readin_dataran(printinfo)
		...
		if(printinfo) then
			...
			write(*,'(23x,A,2f12.3,A)') '   Redshift range: (',gb_redcut_min,gb_redcut_max, ')'
