...
100		continue
  ...
  !---------------------------------------------------------------
  	subroutine eqvl_binned_quan_old(quan_val_list, quan_r_list, rmin, rmax, ndat, num_bins, &
  		...
  		real(dl), intent(in) :: rmin, rmax
  		integer, intent(in) :: ndat, num_bins
  		real(dl), intent(out) :: quan_val_av_list(num_bins), quan_val_er_list(num_bins)
                real(dl), intent(out) :: r_av_list(num_bins),  quan_val_var_list(num_bins)
  		! Local
...
!  		endif
  ...
  !---------------------------------------------------------------  	
	subroutine init_RDSR()
		character(len=char_len) :: readinfile = "/Users/forero/Dropbox/BetaSkeletonStandAlone/src/DEC_vs_RAspan-to-DECspan.txt"
		integer :: i
